...
p.value=1-pchisq(deviance(best.fit.b),best.fit.b$df.residuals)
```
...
predcc <- predict(glm.fitcr, type="response")
...
models.table <- data.frame(type=c("Full","choice-educated","choice-random","Backward Selected"),
...
pglm.table <- data.frame(t(data.frame(Type = c("Lasso", "Ridge"), Testing_error = c("0.255", "0.253"), Training_error = c("0.249", "0.248"))))
names(pglm.table) <- NULL
kable(pglm.table) %>% kable_styling(font_size = 14, full_width = FALSE) %>% column_spec(1:3, bold = TRUE, color = "black") 
```

```{r}
cbind(coef(mod.lasso), coef(mod.ridge))
```
